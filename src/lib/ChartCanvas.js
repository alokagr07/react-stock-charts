...
function getCursorStyle() {
	const tooltipStyle = `
	.react-stock-charts-grabbing-cursor {
		pointer-events: all;
		cursor: -moz-grabbing;
		cursor: -webkit-grabbing;
		cursor: grabbing;
	}
	.react-stock-charts-crosshair-cursor {
		pointer-events: all;
		cursor: crosshair;
	}
	.react-stock-charts-tooltip-hover {
		pointer-events: all;
		cursor: pointer;
	}
	.react-stock-charts-avoid-interaction {
		pointer-events: none;
	}
	.react-stock-charts-enable-interaction {
		pointer-events: all;
	}
	.react-stock-charts-tooltip {
		pointer-events: all;
		cursor: pointer;
	}
	...
	.react-stock-charts-pointer-cursor {
		cursor: pointer;
	}
			...
			<div style={{ position: "relative", width, height }} className={className} onClick={onSelect}>
				...
				<svg className={className} width={width} height={height} style={{ position: "absolute", zIndex: (zIndex + 5) }}>
					...
					<g transform={`translate(${margin.left + 0.5}, ${margin.top + 0.5})`}>
						...
						<g className="react-stock-charts-avoid-interaction">
							{this.props.children}
						</g>
					</g>
				</svg>
